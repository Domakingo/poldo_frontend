...
const minPrice = computed(() => Math.min(
  ...productsStore.products.map(p => p.price)
))
...
const filteredProducts = computed(() => {
  return productsStore.products.filter(product => {
    const matchesPrice =
      product.price >= activeFilters.value.prezzo.min &&
      ...
      <template v-else>
        <div v-for="(products, macro) in groupedByMacro" :key="macro">
          ...
          <CardGrid :minWidth="'280px'">
            <CardProdotto v-for="product in products" :key="product.id" v-bind="{ ...product, productId: product.id }"
              :disabled="haveCart" />
          </CardGrid>
        </div>
      </template>
